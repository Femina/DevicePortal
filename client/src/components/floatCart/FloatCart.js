...
class FloatCart extends Component {
  ...
  addProduct = (product) => {
    ...
    cartProducts.forEach(cp => {
      if (cp.DeviceName === product.DeviceName) {
  ...
  removeProduct = (product) => {
    ...
    const index = cartProducts.findIndex(p => p.DeviceName === product.DeviceName);
    if (index >= 0) {
  ...
  proceedToReturn = () => {
    ...
    if (!productQuantity) {
      alert("Book Some Device!");
    }else {
      alert(`Returned Device. Price: ${currencyFormat} ${util.formatPrice(totalPrice, currencyId)}`);
      this.closeFloatCart();
      this.props.loadCart( [] );
  ...
  proceedToCheckout = () => {
    ...
    if (!productQuantity) {
      ...
      alert(`Booked Device from IP Address: ${this.state.ip} @${curTime}. Price: ${currencyFormat} ${util.formatPrice(totalPrice, currencyId)}`);
    }
  }
      ...
      <div className={classes.join(' ')}>
        ...
        <div className="float-cart__content">
          ...
          <div className="float-cart__shelf-container">
            ...
            {!products.length && (
              <p className="shelf-empty">
                It's Empty. Book Some Devices :) <br />:)
              </p>
            )}
          </div>
          ...
          <div className="float-cart__footer">
            ...
            <div onClick={() => this.proceedToReturn()} className="buy-btn">
              Return
            </div>
          </div>
        </div>
